...
def BAMmain():
    ...
    def compute_gauss(k, j, i):
        if abs(_lambda[k] - CRwasub[j][i]) > 70.0:
            return 0.0
        else:
    ...
    for i in range(1):#endboundsamp):
        ...
        for k in range(waves):
            for j in range(endboundsubband):
                #print(f"Difference: {abs(_lambda[k] - CRwasub[j][i])}")
        ...
        for l in range(endboundline):
            ...
            for j in range(1, xcols):
                for k in range(endboundsubband):
                    ...
                    for m in range(waves):
                        ...
                        #print(f"Out: of j:{j} k:{k} = {out[j][k]}")
            ...
            ssa[i][l][:endboundsubband] = xssa[:endboundsubband]
    ...
    np.save("SSA_Numpy.npy", np.array(ans))
    return
